...
| Per-terminal history clear | The Clear terminal control opened the browser-native confirmation “Clear the history for SK Shell?”; accepting it removed the active shell transcript while the IDE remained interactive. | Live browser verified. |
...
| Multi-line terminal input | SK Shell uses a native multi-line composer with paste, copy, select-all, touch selection, Enter-to-send, Shift+Enter line breaks, and a 180 px maximum expansion. | Live browser verified. |
