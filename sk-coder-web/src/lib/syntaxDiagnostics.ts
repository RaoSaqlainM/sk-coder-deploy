...
export function analyzeSourceSyntax(content: string, language: string | undefined, path: string): ErrorEntry[] {
  if (!supported.has((language || "").toLowerCase())) return []
  ...
  for (let lineIndex = 0; lineIndex < lines.length; lineIndex++) {
    ...
    for (let index = 0; index < line.length; index++) {
      ...
      if (!quote && char === "/" && next === "*") { inBlockComment = true; index++; continue }
      if (!quote && ((char === "/" && next === "/") || (char === "#" && ["python", "ruby"].includes((language || "").toLowerCase())))) break
      if (quote) {
      ...
      if (char === "(" || char === "[" || char === "{") stack.push({ char, line: lineNumber, col })
      if (closers[char]) {
        const open = stack.at(-1)
        if (!open || open.char !== closers[char]) diagnostics.push(entry(path, lineNumber, col, `Unexpected '${char}'`))
        else stack.pop()
      }
    }
    if (quote && quote !== "`") {
  ...
  if (inBlockComment) diagnostics.push(entry(path, lines.length, Math.max(1, lines.at(-1)?.length || 1), "Unterminated block comment"))
  for (const open of stack.slice(-20)) diagnostics.push(entry(path, open.line, open.col, `Unclosed '${open.char}'`))
