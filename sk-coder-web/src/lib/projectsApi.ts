...
export async function updateProject(
  id: number,
  data: { name?: string; description?: string; language?: string; files?: FileNode[] }
): Promise<CloudProject> {
...
export function detectLanguage(files: FileNode[]): string {
  ...
  if (exts.includes("html")) return "html";
  if (exts.includes("css")) return "css";
  if (exts.includes("rs")) return "rust";
  if (exts.includes("go")) return "go";
  if (exts.includes("cpp") || exts.includes("c")) return "cpp";
