...
import { getFileCapability, isDirectPreviewFile, previewLabelFor } from "@/lib/projectCapabilities"
...
export default function ContextMenu() {
  ...
  useEffect(() => {
    try {
      ...
      // ignore persisted clipboard errors
    }
  }, [])
  ...
  function handlePasteHere() {
    if (!node || node.type !== "folder" || !clipboardState) return
  ...
  return (
    <div className="context-menu" ref={ref} style={{ left: menuLeft, top: menuTop }}>
      ...
      <div
        className="context-menu-item context-submenu-trigger"
        onMouseEnter={() => setTermSubMenu(true)}
        onMouseLeave={() => setTermSubMenu(false)}
      >
        ...
        {termSubMenu && (
          <div className={`context-submenu ${submenuLeft === "auto" ? "flip-left" : ""}`}>
            {TERM_OPTS.map((opt) => (
              <div
                key={opt.type}
                className="context-menu-item"
                onClick={() => handleOpenInTerminal(opt.type)}
              >
      ...
      {node?.type === "file" && fileCapability !== "none" && (
        <>
      ...
      {clipboardState && node?.type === "folder" && (
        <div className="context-menu-item" onClick={handlePasteHere}>
