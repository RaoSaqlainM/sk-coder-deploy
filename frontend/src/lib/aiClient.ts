...
function detectEndpoint(key: string, customEndpoint: string): {
    ...
    if (customEndpoint.trim()) {
        ...
        if (ep.includes("generativelanguage.googleapis.com"))
            return { endpoint: ep, type: "gemini" };
        return { endpoint: ep, type: "openai" };
    }
    if (key.startsWith("AIza"))
        return { endpoint: "gemini", type: "gemini" };
    if (key.startsWith("gsk_"))
        return { endpoint: "https://api.groq.com/openai/v1", type: "openai" };
...
async function callOpenAICompat(key: string, endpoint: string, model: string, messages: AIChatMessage[], systemPrompt: string): Promise<AIResponse> {
    try {
