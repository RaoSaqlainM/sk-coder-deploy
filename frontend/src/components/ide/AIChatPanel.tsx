...
async function sendViaPuter(prompt: string): Promise<string> {
    ...
    if (!ok)
        return "Puter.js failed to load. Check your internet connection.";
    try {
...
export default function AIChatPanel() {
    const { aiChatMessages, aiTyping, settings, addAIChatMessage, clearAIChat, setAITyping, setShowSettings, setSettingsTab, getActiveFile, fileTree, addFile, updateFileContent, deleteNode, setTerminalBridgeCmd, setActivePanel, openTab, setPreviewResult, setIsRunning, } = useIDEStore();
    ...
    function deliverAIReply(reply: string) {
        ...
        if (actions.length)
            setProposals((previous) => [...previous, ...actions]);
    }
    ...
    return (<div className="ai-chat-panel">
      <div className="ai-chat-header">
        <div style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
          ...
          {!usePuter && keyStatus === "valid" && (<span className="badge badge-green" style={{ fontSize: 9 }}>Active</span>)}
        </div>
        <div style={{ display: "flex", gap: 4 }}>
          ...
          <button className="btn-icon" onClick={clearAIChat} title="Clear chat" disabled={aiChatMessages.length === 0}>
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
      ...
      {noKey && (<div className="ai-no-key-notice" onClick={() => { setSettingsTab("ai"); setShowSettings(true); }}>
        ...
        {aiChatMessages.map((msg) => (<div key={msg.id} className={`ai-chat-message ${msg.role}`}>
            <div className={`ai-chat-avatar ${msg.role}`}>
              {msg.role === "user" ? (<svg width="12" height="12" viewBox="0 0 24 24" fill="currentColor">
                ...
                </svg>) : (<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                  <rect x="3" y="3" width="18" height="18" rx="4"/>
                  <path d="M9 9h.01M15 9h.01M9 15h6"/>
                </svg>)}
            </div>
            <div className="ai-chat-bubble">
              {msg.role === "assistant" ? (<ReactMarkdown remarkPlugins={[remarkGfm]} components={{
                    code({ children, className }) {
                        ...
                        return isBlock ? (<pre><code className={className}>{children}</code></pre>) : (<code>{children}</code>);
                    },
                }}>
                  {msg.content}
      ...
      <div className="ai-chat-input-area">
        {activeFile && settings.ai.autoContext && (<div className="ai-chat-context-tag">
        ...
        <div className="ai-chat-input-row">
          <textarea ref={textareaRef} value={input} onChange={handleTextareaChange} onKeyDown={handleKeyDown} placeholder={noKey ? "Add an AI key or enable the free AI Assistant in Settings..." : "Describe the task, paste code, or request an approved workspace action…"} disabled={aiTyping} rows={1} aria-label="SK Coder AI Assistant message"/>
