...
export default function ApkEditor() {
    ...
    return (<div style={{ display: "flex", flexDirection: "column", height: "100%", background: "var(--bg-primary)", overflow: "hidden" }}>
      <div style={{
        ...
        <div style={{ flex: 1 }}/>
        {files.length > 0 && (<>
            <button className="btn btn-primary" onClick={repackage} disabled={loading} style={{ fontSize: 11, padding: "0.2rem 0.6rem", flexShrink: 0 }}>
              {loading ? "Packing..." : "↓ Repackage"}
            </button>
            <button className="btn btn-ghost" onClick={() => inputRef.current?.click()} style={{ fontSize: 11, padding: "0.2rem 0.5rem", flexShrink: 0 }}>
              Open
            </button>
        ...
        <input ref={replacementInputRef} type="file" accept="image/png,image/jpeg,image/gif,image/webp,image/bmp,image/svg+xml" style={{ display: "none" }} onChange={(event) => {
      ...
      {files.length === 0 ? (<div onDrop={handleDrop} onDragOver={(e) => e.preventDefault()} style={{
          ...
          <div style={{ display: "flex", gap: "0.5rem", flexWrap: "wrap", justifyContent: "center", maxWidth: 320 }}>
            {["1 Browse archive", "2 Edit text resources", "3 Replace image resources", "4 Rebuild & download"].map((f) => (<span key={f} style={{
                    fontSize: 10, padding: "0.2rem 0.5rem", borderRadius: 4,
                    background: "var(--bg-elevated)", color: "var(--text-secondary)",
                    border: "1px solid var(--border)",
                }}>{f}</span>))}
          </div>
        </div>) : (<div style={{ flex: 1, display: "flex", minHeight: 0, overflow: "hidden" }}>
          <div style={{ width: "38%", borderRight: "1px solid var(--border)", display: "flex", flexDirection: "column", minHeight: 0 }}>
            <div style={{ padding: "0.4rem 0.5rem", borderBottom: "1px solid var(--border-subtle)", flexShrink: 0 }}>
              <input type="text" placeholder="Search files..." value={search} onChange={(e) => setSearch(e.target.value)} style={{ fontSize: 11, padding: "0.2rem 0.5rem" }}/>
            </div>
          ...
          <div style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0 }}>
            {selected ? (<>
                <div style={{
                  ...
                  {selected.isText && (<button className="btn btn-primary" onClick={saveEdit} disabled={saving} style={{ fontSize: 10, padding: "0.15rem 0.5rem", flexShrink: 0 }}>
                      {saving ? "Saving..." : "Save"}
                    </button>)}
                  {isImageFile(selected.path) && (<button className="btn btn-primary" onClick={() => replacementInputRef.current?.click()} style={{ fontSize: 10, padding: "0.15rem 0.5rem", flexShrink: 0 }}>
                      Replace image
                    </button>)}
                </div>
                {selected.isText ? (<textarea value={editContent} onChange={(e) => setEditContent(e.target.value)} style={{
