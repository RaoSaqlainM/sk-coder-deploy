...
export default function FileExplorer() {
    const { fileTree, setNewItem, importFiles, getActiveFile, selectionMode, selectedPaths, batchOperation, setSelectionMode, clearSelectedPaths, setBatchOperation, deleteNodes, } = useIDEStore();
    ...
    return (<div className="file-explorer" onDragOver={(e) => {
      ...
      <input ref={importInputRef} type="file" multiple style={{ display: "none" }} onChange={async (e) => { if (e.target.files)
        await handleSmartImport(e.target.files); e.target.value = ""; }}/>
      <input ref={archiveInputRef} type="file" accept=".zip,.jar,.apk,.xapk,.apks,.war,.ear,.aar" style={{ display: "none" }} onChange={async (e) => { if (e.target.files)
        await handleSmartImport(e.target.files); e.target.value = ""; }}/>
    </div>);
}
